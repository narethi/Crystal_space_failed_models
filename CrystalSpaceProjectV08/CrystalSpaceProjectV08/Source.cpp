...
//Height map information from libnoise
module::Perlin mapSeed;
...
struct sphere
{
	...
	//This is where the heightmap is placed
	double* noiseSeed;
...
sphere generateNoiseMap(sphere world)
{
	srand(time(NULL));
	world.noiseSeed[0] = rand()%10000/1000;
	srand(rand()%10000); 
	world.noiseSeed[1] = rand()%10000/1000;
	srand(rand()*5); 
	world.noiseSeed[2] = rand()%10000/1000;
	srand(rand()); 
	world.noiseSeed[3] = rand()%10000/1000;
	srand(rand()*25);
	world.noiseSeed[4] = rand()%10000/1000;
	srand(rand()); 
	world.noiseSeed[5] = rand()%10000/1000;
	return world;
}
...
sphere initializeSphere(int resolution, double radius)
{
	...
	world.colors0 = new double[count];
	world.colors1 = new double[count];
	world.colors2 = new double[count];
	world.colors3 = new double[count];
	world.colors4 = new double[count];
	world.colors5 = new double[count];
	...
	world.noiseSeed = new double[6];

	world = generateNoiseMap(world);
...
sphere generateGrid(double radius, int resolution /*,heightMap*/)
{
	...
	double step = PI2/pow(2, resolution), check = 0, modifier;
	...
	*/
	...
	for(double i = 0; i < pow(2, resolution); i++)
	{
		for(double j = 0; j < pow(2, resolution) + 1; j++)
		{
			...
			{
				world.colors0[count]		= j/(pow(2, resolution) + 1);
				world.colors0[count + 1] = 0;
				world.colors0[count + 2] = 0;

				world.colors0[count + 3] = (j + 1)/(pow(2, resolution) + 1);
				world.colors0[count + 4] = 0;
				world.colors0[count + 5] = 0;
			...
			else if(j > world.intersection0[int(i*5) + 1] && j < world.intersection0[int(i*5) + stageIntersection + 1])
			{
				...
				mapSeed.SetFrequency(world.noiseSeed[0]);
				if (i > 0) 
					modifier = mapSeed.GetValue(i/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				else modifier = 1;

				world.colors0[count]	= mapSeed.GetValue(i/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				world.colors0[count + 1] = 0;
				world.colors0[count + 2] = 0;

				world.colors0[count + 3] = mapSeed.GetValue((i+1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				world.colors0[count + 4] = 0;
				world.colors0[count + 5] = 0;

				world.plane0[count]		= (modifier*radius/10 + radius)*sin(check + PI4)*sin(i*step + 7*PI4);
				world.plane0[count + 1] = (modifier*radius/10 + radius)*sin(check + PI4)*cos(i*step + 7*PI4); 
				world.plane0[count + 2] = (modifier*radius/10 + radius)*cos(check + PI4);

				if (i < pow(2, resolution) - 1) 
					modifier = mapSeed.GetValue((i+1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				else modifier = 1;

				world.plane0[count + 3] = (modifier*radius/10 + radius)*sin(check + PI4)*sin((i + 1)*step + 7*PI4);
				world.plane0[count + 4] = (modifier*radius/10 + radius)*sin(check + PI4)*cos((i + 1)*step + 7*PI4); 
				world.plane0[count + 5] = (modifier*radius/10 + radius)*cos(check + PI4);

				//Plane 1
				mapSeed.SetFrequency(world.noiseSeed[1]);
				if (i > 0)
					modifier = mapSeed.GetValue(i/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				else modifier = 1;

				world.colors1[count]	= mapSeed.GetValue(i/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				world.colors1[count + 1] = 0;
				world.colors1[count + 2] = 1;

				world.colors1[count + 3] = mapSeed.GetValue((i+1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				world.colors1[count + 4] = 0;
				world.colors1[count + 5] = 1;

				world.plane1[count]		= (modifier*radius/10 + radius)*sin(check + PI4)*sin(i*step + 3*PI4);
				world.plane1[count + 1] = (modifier*radius/10 + radius)*sin(check + PI4)*cos(i*step + 3*PI4); 
				world.plane1[count + 2] = (modifier*radius/10 + radius)*cos(check + PI4);

				if (i < pow(2, resolution) - 1) 
					modifier = mapSeed.GetValue((i+1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				else modifier = 1;

				world.plane1[count + 3] = (modifier*radius/10 + radius)*sin(check + PI4)*sin((i + 1)*step + 3*PI4);
				world.plane1[count + 4] = (modifier*radius/10 + radius)*sin(check + PI4)*cos((i + 1)*step + 3*PI4); 
				world.plane1[count + 5] = (modifier*radius/10 + radius)*cos(check + PI4);

				//Plane 2
				mapSeed.SetFrequency(world.noiseSeed[2]);
				if (i > 0)
					modifier = mapSeed.GetValue(i/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				else modifier = 1;

				world.colors2[count]	= mapSeed.GetValue(i/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				world.colors2[count + 1] = 1;
				world.colors2[count + 2] = 0;

				world.colors2[count + 3] = mapSeed.GetValue((i+1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				world.colors2[count + 4] = 1;
				world.colors2[count + 5] = 0;

				world.plane2[count]		= (modifier*radius/10 + radius)*sin(check + PI4)*cos(i*step + 7*PI4);
				world.plane2[count + 1] = (modifier*radius/10 + radius)*cos(check + PI4); 
				world.plane2[count + 2] = (modifier*radius/10 + radius)*sin(check + PI4)*sin(i*step + 7*PI4);

				if (i < pow(2, resolution) - 1) 
					modifier = mapSeed.GetValue((i+1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				else modifier = 1;

				world.plane2[count + 3] = (modifier*radius/10 + radius)*sin(check + PI4)*cos((i + 1)*step + 7*PI4);
				world.plane2[count + 4] = (modifier*radius/10 + radius)*cos(check + PI4); 
				world.plane2[count + 5] = (modifier*radius/10 + radius)*sin(check + PI4)*sin((i + 1)*step + 7*PI4);

				//Plane 3
				mapSeed.SetFrequency(world.noiseSeed[3]);
				if (i > 0)
					modifier = mapSeed.GetValue(i/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				else modifier = 1;

				world.colors3[count]	 = 0;
				world.colors3[count + 1] = 0;
				world.colors3[count + 2] = mapSeed.GetValue(i/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));

				world.colors3[count + 3] = 0;
				world.colors3[count + 4] = 0;
				world.colors3[count + 5] = mapSeed.GetValue((i+1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));

				world.plane3[count]		= (modifier*radius/10 + radius)*sin(check + PI4)*cos(i*step + 3*PI4);
				world.plane3[count + 1] = (modifier*radius/10 + radius)*cos(check + PI4); 
				world.plane3[count + 2] = (modifier*radius/10 + radius)*sin(check + PI4)*sin(i*step + 3*PI4);
				
				if (i < pow(2, resolution) - 1) 
					modifier = mapSeed.GetValue((i+1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				else modifier = 1;

				world.plane3[count + 3] = (mapSeed.GetValue((i + 1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1))*radius/10 + radius)*sin(check + PI4)*cos((i + 1)*step + 3*PI4);
				world.plane3[count + 4] = (mapSeed.GetValue((i + 1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1))*radius/10 + radius)*cos(check + PI4); 
				world.plane3[count + 5] = (mapSeed.GetValue((i + 1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1))*radius/10 + radius)*sin(check + PI4)*sin((i + 1)*step + 3*PI4);

				//Plane 4
				mapSeed.SetFrequency(world.noiseSeed[4]);

				world.colors4[count]	= 0;
				world.colors4[count + 1] = mapSeed.GetValue(i/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				world.colors4[count + 2] = 0;

				world.colors4[count + 3] = 0;
				world.colors4[count + 4] = mapSeed.GetValue((i+1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				world.colors4[count + 5] = 0;

				world.plane4[count]		= (mapSeed.GetValue(i/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1))*radius/10 + radius)*cos(check + PI4);
				world.plane4[count + 1] = (mapSeed.GetValue(i/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1))*radius/10 + radius)*sin(check + PI4)*cos(i*step + PI4); 
				world.plane4[count + 2] = (mapSeed.GetValue(i/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1))*radius/10 + radius)*sin(check + PI4)*sin(i*step + PI4);

				world.plane4[count + 3] = (mapSeed.GetValue((i + 1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1))*radius/10 + radius)*cos(check + PI4);
				world.plane4[count + 4] = (mapSeed.GetValue((i + 1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1))*radius/10 + radius)*sin(check + PI4)*cos((i + 1)*step + PI4); 
				world.plane4[count + 5] = (mapSeed.GetValue((i + 1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1))*radius/10 + radius)*sin(check + PI4)*sin((i + 1)*step + PI4);

				//Plane 5
				mapSeed.SetFrequency(world.noiseSeed[5]);

				world.colors5[count]	= mapSeed.GetValue(i/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				world.colors5[count + 1] = .5;
				world.colors5[count + 2] = .5;

				world.colors5[count + 3] = mapSeed.GetValue((i+1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1));
				world.colors5[count + 4] = .5;
				world.colors5[count + 5] = .5;

				world.plane5[count]		= (mapSeed.GetValue(i/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1))*radius/10 + radius)*cos(check + 5*PI4);
				world.plane5[count + 1] = (mapSeed.GetValue(i/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1))*radius/10 + radius)*sin(check + 5*PI4)*cos(i*step + PI4); 
				world.plane5[count + 2] = (mapSeed.GetValue(i/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1))*radius/10 + radius)*sin(check + 5*PI4)*sin(i*step + PI4);

				world.plane5[count + 3] = (mapSeed.GetValue((i + 1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1))*radius/10 + radius)*cos(check + 5*PI4);
				world.plane5[count + 4] = (mapSeed.GetValue((i + 1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1))*radius/10 + radius)*sin(check + 5*PI4)*cos((i + 1)*step + PI4); 
				world.plane5[count + 5] = (mapSeed.GetValue((i + 1)/pow(2, resolution), check/(pow(2, resolution)*step), 1/pow(2, resolution + 1))*radius/10 + radius)*sin(check + 5*PI4)*sin((i + 1)*step + PI4);
...
void display(void)
{
	...
	glColorPointer(3, GL_DOUBLE, 0, greg.colors0);
	glDrawArrays(GL_TRIANGLE_STRIP, 0, (greg.size/3));
	glColorPointer(3, GL_DOUBLE, 0, greg.colors1);
	glVertexPointer(3, GL_DOUBLE, 0, greg.plane1);
	glDrawArrays(GL_TRIANGLE_STRIP, 0, (greg.size/3));
	glColorPointer(3, GL_DOUBLE, 0, greg.colors2);
	glVertexPointer(3, GL_DOUBLE, 0, greg.plane2);
	glDrawArrays(GL_TRIANGLE_STRIP, 0, (greg.size/3));
	glColorPointer(3, GL_DOUBLE, 0, greg.colors3);
	glVertexPointer(3, GL_DOUBLE, 0, greg.plane3);
	glDrawArrays(GL_TRIANGLE_STRIP, 0, (greg.size/3));
	glColorPointer(3, GL_DOUBLE, 0, greg.colors4);
	glVertexPointer(3, GL_DOUBLE, 0, greg.plane4);
	glDrawArrays(GL_TRIANGLE_STRIP, 0, (greg.size/3));
	glColorPointer(3, GL_DOUBLE, 0, greg.colors5);
